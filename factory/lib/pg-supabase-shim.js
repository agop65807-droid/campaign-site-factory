...
function getPool() {
  ...
  pool = new Pool({
    ...
    // A low per-instance max avoids exhausting Postgres's max_connections
    // (especially relevant on Render's smaller Postgres plans). If you outgrow
    // this, put PgBouncer (or Render's own connection pooling, where available)
    // in front instead of just raising this number.
    max: 3,
