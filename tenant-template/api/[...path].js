...
// db.js implements a small, fluent Postgres query builder (db.from('table')
// .select().eq()...) backed by `pg` — see lib/db.js and lib/db.test.js.
const { createClient } = require('../lib/db');

// DATABASE_URL is the only thing this needs now — a Render Postgres connection
// string (Render dashboard → your database → "External Database URL" for local
// dev, or the "Internal Database URL" when this runs as a Vercel function,
// which needs the external one since Vercel isn't on Render's private network).
...
// Fallback identity — used by /api/config whenever a `site_settings` row does not exist
// yet (e.g. this migration hasn't been run, or this is still the original single-tenant
// deployment). Keeping this in sync with the current hardcoded branding means adopting
// the dynamic-config system below never changes behavior for the live site until someone
// actually creates/edits a site_settings row.
const DEFAULT_SITE_CONFIG = {
  orgName: 'لجنة اعتصام أبناء المهرة',
  siteTitle: 'السعودية سبب الفتن',
  siteDescription: 'السعودية سبب الفتن - الحملة الإلكترونية الموحدة',
  hashtag: '',
  ...
  themeMode: 'dark',
  enabledSharePlatforms: ['x', 'whatsapp', 'facebook'],
  socialLinks: {}
};
...
// ============================================================================
// ACCOUNT LOCKOUT (security §11 item 7) — per-username, on top of the per-IP limiter
// ============================================================================

const ACCOUNT_LOCKOUT_WINDOW = 15 * 60 * 1000;
...
function lockoutKey(username) {
  return crypto.createHash('sha256').update(String(username).toLowerCase().trim()).digest('hex').slice(0, 32);
}
...
function recordFailedLogin(username) {
  const key = lockoutKey(username);
  const now = Date.now();
  const record = accountLockoutStore.get(key) || { count: 0, firstFailure: now };
  if (now - record.firstFailure > ACCOUNT_LOCKOUT_WINDOW) {
    record.count = 0;
    record.firstFailure = now;
  }
  record.count++;
  if (record.count >= ACCOUNT_LOCKOUT_MAX_FAILURES) {
    record.lockedUntil = now + ACCOUNT_LOCKOUT_DURATION;
  }
  accountLockoutStore.set(key, record);
}

function clearAccountLockout(username) {
  accountLockoutStore.delete(lockoutKey(username));
}
...
async function handleAuth(req, res) {
  ...
  try {
    ...
    // Per-account lockout (security §11 item 7), on top of the existing per-IP rate limit
    // above. Note: like the IP rate limiter, this in-memory store is per serverless
    // instance and resets on cold start — adequate as a first line of defense, but for a
    // durable guarantee this should move to a DB-backed counter (e.g. on sub_admins /
    // main_admins) in a follow-up.
    const lockCheck = checkAccountLockout(username);
    if (!lockCheck.allowed) {
      res.writeHead(429, { ...corsHeaders, 'Retry-After': String(lockCheck.retryAfter) });
      res.end(JSON.stringify({ error: 'Account temporarily locked due to repeated failed logins. Please try again later.', retryAfter: lockCheck.retryAfter }));
      return;
    }

    let authResult = null;

    // Try main admin — DB-backed (main_admins, PBKDF2-hashed) first. This is the Phase 2
    // storage path (security §11 item 4): main admin credentials created via the Factory
    // wizard live here instead of as a plaintext env var. Falls back to ADMIN_USER/
    // ADMIN_PASS below for any tenant that hasn't been migrated to a main_admins row yet,
    // so the current live site keeps working unchanged.
    try {
      const { data: mainAdmin, error: maErr } = await db
        .from('main_admins')
        .select('*')
      ...
      if (!maErr && mainAdmin) {
        ...
        if (timingSafeCompare(hashedInput, mainAdmin.password_hash)) {
          authResult = { success: true, adminType: 'main', name: mainAdmin.name || 'المشرف الرئيسي', mustChangePassword: !!mainAdmin.must_change_password };
          await db.from('main_admins').update({ last_login_at: new Date().toISOString() }).eq('id', mainAdmin.id);
        }
      }
    } catch (e) {
      // main_admins table may not exist yet (migration 002 not applied) — fall through
      // to the legacy env-var path below rather than failing the login.
    }

    // Legacy/fallback main admin path — plaintext env vars (original v4 behavior).
    if (!authResult) {
    ...
    // Try sub-admin
    if (!authResult) {
      const { data: subAdmin, error } = await db
        .from('sub_admins')
        .select('*')
        .eq('username', username)
        .eq('is_active', true)
      ...
      if (!error && subAdmin) {
        ...
        if (timingSafeCompare(hashedInput, subAdmin.password_hash)) {
          await db.from('sub_admins').update({ last_login_at: new Date().toISOString() }).eq('id', subAdmin.id);
          authResult = {
...
async function handleLogout(req, res) {
  ...
  try {
    ...
    if (token && token.length >= 32) {
      const tokenHash = hashToken(token);
      await db.from('admin_sessions').update({ revoked_at: new Date().toISOString() }).eq('session_token_hash', tokenHash);
    }
    res.writeHead(200, corsHeaders);
...
async function handleCampaign(req, res) {
  ...
  try {
    ...
    if (campaignId) {
      ...
      if (!campaign) {
        if (!isPublic) {
          const { data: directCampaign } = await db.from('campaigns').select('*').eq('id', parseInt(campaignId, 10)).single();
          if (directCampaign) {
            const { data: tweetsData } = await db.from('tweets').select('*').eq('campaign_id', directCampaign.id).order('created_at', { ascending: false });
            res.writeHead(200, corsHeaders);
      ...
      const { data: tweetsData } = await db.from('tweets').select('*').eq('campaign_id', campaign.id).order('created_at', { ascending: false });
      res.writeHead(200, corsHeaders);
    ...
    let tweets = [];
    if (firstCampaign) {
      const { data: tweetsData } = await db.from('tweets').select('*').eq('campaign_id', firstCampaign.id).order('created_at', { ascending: false });
      tweets = tweetsData || [];
    }
...
async function handleCampaignUpdate(parsed, user, req) {
  ...
  const { data, error } = await db.from('campaigns').upsert(upsertData, { onConflict: 'id' }).select().single();
...
async function handleTweetInsert(parsed, user, req) {
  ...
  if (tweetId) {
    ...
    const { data, error } = await db.from('tweets').update(insertData).eq('id', insertData.id).select().single();
    if (error) throw error;
    ...
    const { data, error } = await db.from('tweets').insert(insertData).select().single();
    if (error) throw error;
...
async function handleExcelImport(parsed, user, req) {
  ...
  const { data, error } = await db.from('tweets').insert(inserts).select();
  if (error) throw error;
...
async function handleDeleteCampaign(req, res) {
  ...
  try {
    ...
    const { data: campaign } = await db.from('campaigns').select('name').eq('id', numericId).single();
    await db.from('tweets').delete().eq('campaign_id', numericId);
    const { error } = await db.from('campaigns').delete().eq('id', numericId);
    if (error) {
      res.writeHead(500, corsHeaders);
...
async function handleDeleteTweet(req, res) {
  ...
  try {
    ...
    const { data: tweet } = await db.from('tweets').select('campaign_id, text').eq('id', numericId).single();
    const { error } = await db.from('tweets').delete().eq('id', numericId);
    if (error) {
      res.writeHead(500, corsHeaders);
...
async function handleSubAdmins(req, res) {
  ...
  try {
    switch (req.method) {
      case 'GET': {
        const { data: subAdmins, error } = await db
          .from('sub_admins')
      ...
      case 'POST': {
        ...
        const { data, error } = await db.from('sub_admins').insert({
      ...
      case 'PUT': {
        ...
        const { data, error } = await db.from('sub_admins').update(updateData).eq('id', parseInt(id)).select('id, name, username, is_active, permissions, created_at, updated_at').single();
        if (error) {
          res.writeHead(500, corsHeaders);
          res.end(JSON.stringify({ error: error.message }));
          return;
      ...
      case 'DELETE': {
        ...
        const { data, error } = await db.from('sub_admins').update({ is_active: false, updated_at: new Date().toISOString() }).eq('id', parseInt(id)).select('id, name, is_active').single();
        if (error) {
          res.writeHead(500, corsHeaders);
          res.end(JSON.stringify({ error: error.message }));
          return;
        ...
        await db.from('admin_sessions').update({ revoked_at: new Date().toISOString() }).eq('sub_admin_id', parseInt(id)).is('revoked_at', null);
  ...
  try {
    switch (req.method) {
      case 'GET': {
        ...
        let query = db.from('invite_links').select('*');
      ...
      case 'PUT': {
        ...
        if (name !== undefined) updateData.name = sanitizeInput(name);
        if (isActive !== undefined) updateData.is_active = isActive;
        const { data, error } = await db.from('invite_links').update(updateData).eq('id', parseInt(id)).select().single();
        if (error) {
          res.writeHead(500, corsHeaders);
          res.end(JSON.stringify({ error: error.message }));
          return;
      ...
      case 'DELETE': {
        ...
        const { data, error } = await db.from('invite_links').update({ is_active: false, updated_at: new Date().toISOString() }).eq('id', parseInt(id)).select().single();
        if (error) {
          res.writeHead(500, corsHeaders);
          res.end(JSON.stringify({ error: error.message }));
          return;
...
// ============================================================================
// HANDLER: /api/config  (Phase 2 foundation — dynamic site identity, FR4)
// ============================================================================
// Public, read-only, unauthenticated. Only ever returns non-sensitive branding/display
// data (name, logo, colors, hashtag, which share platforms are on). Never put secrets,
// tokens, or admin info here. Used by index.html (to render identity + toggle share
// buttons), admin.html (branding preview), and sw.js (push notification title/icon).
...
async function handleConfig(req, res) {
  ...
  try {
    ...
    // PGRST116 = no rows found (table exists but empty) — fall back silently.
    // Any other error (e.g. table doesn't exist because migration 002 wasn't run yet)
    // also falls back silently rather than breaking the public homepage.
    if (!error && row) {
      config = {
        orgName: row.org_name || DEFAULT_SITE_CONFIG.orgName,
        siteTitle: row.site_title || row.org_name || DEFAULT_SITE_CONFIG.siteTitle,
        siteDescription: row.site_description || DEFAULT_SITE_CONFIG.siteDescription,
        hashtag: row.hashtag || DEFAULT_SITE_CONFIG.hashtag,
        logoUrl: row.logo_url || DEFAULT_SITE_CONFIG.logoUrl,
        faviconUrl: row.favicon_url || row.logo_url || DEFAULT_SITE_CONFIG.faviconUrl,
        primaryColor: row.primary_color || DEFAULT_SITE_CONFIG.primaryColor,
        secondaryColor: row.secondary_color || DEFAULT_SITE_CONFIG.secondaryColor,
        themeMode: row.theme_mode || DEFAULT_SITE_CONFIG.themeMode,
        enabledSharePlatforms: Array.isArray(row.enabled_share_platforms) && row.enabled_share_platforms.length
          ? row.enabled_share_platforms
          : DEFAULT_SITE_CONFIG.enabledSharePlatforms,
        socialLinks: row.social_links || {}
      };
    }
  } catch (e) {
    console.error('Config lookup failed, using defaults:', e.message);
  }

  // Cacheable for a short window: identity rarely changes, and this is on the hot path
  // of every page load. Overrides the default no-store CORS header for this route only.
  res.writeHead(200, { ...corsHeaders, 'Cache-Control': 'public, max-age=60, s-maxage=300' });
  res.end(JSON.stringify({ success: true, config }));
}

// ============================================================================
// HANDLER: /api/logo  (serves the tenant's uploaded logo straight from the
// database — see migration 003). Falls back to a 404 if nothing was uploaded —
// callers should fall back to site_settings.logo_url (an external image URL)
// in that case, which the Factory wizard also supports as an alternative to
// uploading a file.
// ============================================================================
...
async function handleLogo(req, res) {
  ...
  try {
    const { data: row, error } = await db
      .from('site_settings')
      .select('logo_data_base64, logo_content_type')
      .limit(1)
      .single();

    if (error || !row || !row.logo_data_base64) {
      res.writeHead(404, corsHeaders);
      res.end(JSON.stringify({ error: 'No logo stored — use site_settings.logo_url instead' }));
      return;
    }

    const buffer = Buffer.from(row.logo_data_base64, 'base64');
    res.writeHead(200, {
      'Content-Type': row.logo_content_type || 'image/png',
      'Cache-Control': 'public, max-age=3600',
      'Access-Control-Allow-Origin': '*'
    });
    res.end(buffer);
  } catch (e) {
    console.error('Logo lookup failed:', e.message);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ error: 'Failed to load logo' }));
  }
}
...
async function handleAnalytics(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      const body = await readBody(req, 10000);
      const data = JSON.parse(body || '{}');
      const { eventType, campaignId, tweetId, inviteCode, visitorId, metadata, platform } = data;
      ...
      const validEventTypes = ['page_view', 'campaign_view', 'tweet_share_click', 'tweet_copy', 'invite_visit', 'report_download'];
      ...
      // Platform is only meaningful for share/copy events (Phase 1: multi-platform sharing).
      // Column is nullable + additive via migration 002, so this stays backward compatible
      // with any environment where the migration hasn't been applied yet (insert simply
      // omits the field in that case, see try/catch below).
      const validPlatforms = ['x', 'whatsapp', 'facebook', 'clipboard'];
      const cleanPlatform = (typeof platform === 'string' && validPlatforms.includes(platform)) ? platform : null;
      ...
      let { error } = await db.from('analytics_events').insert(insertData);
      if (error && /column .*platform.* does not exist/i.test(error.message || '')) {
        // Migration 002 (platform column) not applied yet on this database — degrade
        // gracefully instead of breaking analytics tracking entirely.
        const { platform: _drop, ...withoutPlatform } = insertData;
        ({ error } = await db.from('analytics_events').insert(withoutPlatform));
      }
      if (error) {
    ...
    try {
      let query = db.from('analytics_events').select('*');
      if (campaignId) query = query.eq('campaign_id', parseInt(campaignId));
      ...
      const summary = {
        ...
        totalShareClicks: allEvents.filter(e => e.event_type === 'tweet_share_click').length,
        ...
        byCampaign: {}, byInviteCode: {}, timeline: {}, byPlatform: {}
      };

      // Per-platform share breakdown (Phase 1: X / WhatsApp / Facebook comparison — FR5).
      const shareEvents = allEvents.filter(e => e.event_type === 'tweet_share_click');
      const platforms = ['x', 'whatsapp', 'facebook'];
      for (const p of platforms) {
        summary.byPlatform[p] = shareEvents.filter(e => e.platform === p).length;
      }
      // Events recorded before the platform column existed (or without a recognized value).
      summary.byPlatform.unknown = shareEvents.filter(e => !platforms.includes(e.platform)).length;
      ...
      for (const cid of campaignIds) {
        ...
        const campaignShareEvents = campaignEvents.filter(e => e.event_type === 'tweet_share_click');
        summary.byCampaign[cid] = {
          ...
          shareClicks: campaignEvents.filter(e => e.event_type === 'tweet_share_click').length,
          ...
          byPlatform: {
            x: campaignShareEvents.filter(e => e.platform === 'x').length,
            whatsapp: campaignShareEvents.filter(e => e.platform === 'whatsapp').length,
            facebook: campaignShareEvents.filter(e => e.platform === 'facebook').length
          }
        };
      }
      ...
      for (const code of inviteCodes) {
        ...
        summary.byInviteCode[code] = {
          ...
          shareClicks: codeEvents.filter(e => e.event_type === 'tweet_share_click').length,
...
async function generateReportHTML(campaignId) {
  const { data: campaign } = await db.from('campaigns').select('*').eq('id', parseInt(campaignId)).single();
  if (!campaign) return null;

  const { data: tweets } = await db.from('tweets').select('*').eq('campaign_id', parseInt(campaignId)).order('created_at', { ascending: false });
  const { data: analytics } = await db.from('analytics_events').select('*').eq('campaign_id', parseInt(campaignId));
  const { data: inviteLinks } = await db.from('invite_links').select('*').eq('campaign_id', parseInt(campaignId));
  const { data: activityLogs } = await db.from('admin_activity_logs').select('*').eq('campaign_id', parseInt(campaignId)).order('created_at', { ascending: false });
  ...
  const shareClicks = allEvents.filter(e => e.event_type === 'tweet_share_click').length;
  ...
  for (const link of (inviteLinks || [])) {
    ...
    const shareClicksFromLink = linkEvents.filter(e => e.event_type === 'tweet_share_click').length;
...
module.exports = async (req, res) => {
  ...
  switch (path) {
    ...
    case '/api/logo': return handleLogo(req, res);
    case '/api/config': return handleConfig(req, res);
