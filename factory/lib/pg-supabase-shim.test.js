// Validates pg-supabase-shim.js's generated SQL and result-shaping against
// every distinct query pattern actually used in api/[...path].js. Mocks
// Pool.prototype.query so this runs without a live database — run it any
// time you touch the shim, and again once against a real Postgres/Render
// database before trusting it in production.
//
// Usage:  npm install && node lib/pg-supabase-shim.test.js

process.env.DATABASE_URL = process.env.DATABASE_URL || 'postgres://test:test@localhost:5432/test';

const { Pool } = require('pg');
const { createClient } = require('./pg-supabase-shim');
...
(async () => {
  ...
  let { data, error } = await supabase.from('main_admins').select('*').eq('username', 'admin').eq('is_active', true).single();
  ...
  canned = { rows: [] };
  ({ data, error } = await supabase.from('main_admins').select('*').eq('username', 'nope').single());
  ...
  await supabase.from('admin_sessions').select('*').eq('session_token_hash', 'abc').is('revoked_at', null).gt('expires_at', nowIso).single();
  ...
  ({ data } = await supabase.from('admin_sessions').select('*, sub_admins(*)').eq('session_token_hash', 'xyz').is('revoked_at', null).single());
  ...
  // Generalized embed with a different related table (factory_sessions -> super_admins)
  canned = { rows: [{ id: 3, super_admin_id: 7, super_admins: { id: 7, name: 'Sara' } }] };
  ({ data } = await supabase.from('factory_sessions').select('*, super_admins(*)').eq('session_token_hash', 'zzz').single());
  check('generalized embed picks correct FK column', lastCall.sql.includes('LEFT JOIN "super_admins" e ON s."super_admin_id" = e.id'));
  check('generalized embed shapes correctly for a different table', data.super_admins && data.super_admins.name === 'Sara');

  // count/head query (used by the factory bootstrap check: "does a super_admin already exist?")
  canned = { rows: [{ count: '2' }] };
  let countResult = await supabase.from('super_admins').select('*', { count: 'exact', head: true });
  ...
  ({ data } = await supabase.from('campaigns').insert({ name: 'Test Campaign', is_active: true }).select().single());
  ...
  ({ data } = await supabase.from('tweets').insert([{ campaign_id: 1, text: 'a' }, { campaign_id: 1, text: 'b' }, { campaign_id: 1, text: 'c' }]).select());
  check('bulk insert returns array of 3', Array.isArray(data) && data.length === 3);

  canned = { rows: [{ id: 5, is_active: false }] };
  ({ data } = await supabase.from('sub_admins').update({ is_active: false, updated_at: nowIso }).eq('id', 5).select('id, is_active').single());
  check('update SQL', lastCall.sql === 'UPDATE "sub_admins" SET "is_active" = $1, "updated_at" = $2 WHERE "id" = $3 RETURNING *');

  canned = { rows: [{}] };
  await supabase.from('sub_admins').update({ permissions: { canAddTweets: true } }).eq('id', 1);
  ...
  await supabase.from('campaigns').upsert({ id: 7, name: 'Updated' }, { onConflict: 'id' }).select().single();
  ...
  ({ data, error } = await supabase.from('sub_admins').insert({ username: 'dup' }).select().single());
