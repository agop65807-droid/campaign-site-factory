// ============================================================================
// secrets_vault helpers — AES-256-GCM encryption for Vercel/Supabase master
...
// `supabase` is the factory's own Supabase client (service role), passed in
// by the caller rather than imported here, to keep this module easy to unit
// test without a live DB connection.

async function getSecret(supabase, keyName) {
...
async function setSecret(supabase, keyName, plaintextValue) {
...
async function deleteSecret(supabase, keyName) {
...
const KEYS = {
  ...
  SUPABASE_ORG_TOKEN: 'supabase_org_access_token',
  SUPABASE_ORG_ID: 'supabase_organization_id',
  tenantSupabaseServiceKey: (tenantId) => `tenant:${tenantId}:supabase_service_key`,
  tenantSupabaseAnonKey: (tenantId) => `tenant:${tenantId}:supabase_anon_key`,
