...
// SUPABASE_KEY is the original name this template used; SUPABASE_SERVICE_ROLE_KEY
// is also accepted since it's the more explicit/conventional name for a service-role
// key. Either way this MUST be the service_role key (not anon) — this server-side
// code needs to bypass RLS to manage sessions, sub-admins, analytics, etc.
...
async function handleAuth(req, res) {
  ...
  try {
    ...
    try {
      const { data: mainAdmin, error: maErr } = await supabase
        .from('main_admins')
        .select('*')
      ...
      if (!maErr && mainAdmin) {
        ...
        if (timingSafeCompare(hashedInput, mainAdmin.password_hash)) {
          ...
          await supabase.from('main_admins').update({ last_login_at: new Date().toISOString() }).eq('id', mainAdmin.id);
        }
      }
    } catch (e) {
...
async function handleAnalytics(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      let { error } = await supabase.from('analytics_events').insert(insertData);
      if (error && /column .*platform.* does not exist/i.test(error.message || '')) {
        ...
        ({ error } = await supabase.from('analytics_events').insert(withoutPlatform));
      }
      if (error) {
